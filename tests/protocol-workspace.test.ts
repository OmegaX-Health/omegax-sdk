...
function writeProtocolWorkspaceFile(
  repoRoot: string,
  relativePath: string,
  contents: string,
) {
...
test('protocol workspace state is stable when the repo has no local changes', () => {
  ...
  try {
    ...
    assert.equal(
      firstState.workspaceFingerprint,
      secondState.workspaceFingerprint,
    );
  } finally {
...
test('tracked staged and unstaged source changes update the workspace fingerprint', () => {
  ...
  try {
    ...
    assert.notEqual(
      unstagedState.workspaceFingerprint,
    ...
    assert.notEqual(
      stagedState.workspaceFingerprint,
      baseline.workspaceFingerprint,
    );
  } finally {
    rmSync(repoRoot, { recursive: true, force: true });
...
test('untracked source files update the workspace fingerprint', () => {
  ...
  try {
    ...
    writeProtocolWorkspaceFile(
      ...
      'export const nextScenario = true;',
    );
    ...
    assert.notEqual(
      updatedState.workspaceFingerprint,
      baseline.workspaceFingerprint,
    );
  } finally {
    rmSync(repoRoot, { recursive: true, force: true });
...
test('generated workspace directories do not affect the workspace fingerprint', () => {
  ...
  try {
    ...
    writeProtocolWorkspaceFile(
      repoRoot,
      'target/deploy/omegax_protocol.so',
      'generated-program-binary',
    );
    writeProtocolWorkspaceFile(
      repoRoot,
      'node_modules/mock-package/index.js',
